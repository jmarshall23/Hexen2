...
============
*/
void SV_PushMove (edict_t *pusher, float movetime, qboolean update_time)
{
	int			i, e;
	edict_t		*check, *block;
...
void SV_Physics_Toss (edict_t *ent)
{
	...
	{
		groundentity = PROG_TO_EDICT(ent->v.groundentity);
		if (groundentity->free)
		{
			ent->v.flags = (int)ent->v.flags & ~FL_ONGROUND;
			groundentity = PROG_TO_EDICT(pr_global_struct->world);
		}
	}
...
// move origin
	...
	trace = SV_PushEntity (ent, move);
//@@@
	if (trace.fraction == 1 && trace.allsolid == 1)
		return;
	if (ent->free)
		return;
...
/*
 * $Log: /H2 Mission Pack/SV_PHYS.C $
 * 
 * 10    3/17/98 6:14p Jmonroe
 * 
 * 9     2/27/98 6:09p Mgummelt
 * 
 * 8     2/09/98 11:46a Jmonroe
 * 
 * 7     2/04/98 12:53a Jmonroe
 * added fastfabs
 * 
 * 6     1/08/98 5:01p Mgummelt
 * 
 * 5     1/08/98 4:25p Mgummelt
 * 
 * 29    9/01/97 2:08a Rjohnson
 * Physics Update
 * 
 * 28    8/31/97 3:44p Rjohnson
 * Update
 * 
 * 27    8/30/97 6:34p Rjohnson
 * Fix for rotating
 * 
 * 26    8/25/97 9:03a Rjohnson
 * Changed warning messages to developer mode
 * 
 * 25    8/21/97 10:32a Rjohnson
 * Disabled error messages for builds
 * 
 * 24    7/15/97 1:29p Rjohnson
 * Updates for water
 * 
 * 23    7/14/97 4:47p Rjohnson
 * Fix for movetype_phase
 * 
 * 22    6/05/97 4:44p Rlove
 * Fly mode is now network friendly
 * 
 * 21    5/30/97 10:33a Rlove
 * 
 * 20    5/23/97 2:13p Rlove
 * 
 * 19    5/22/97 11:59a Rjohnson
 * Removed the physics hack I put in a few days ago
 * 
 * 18    5/22/97 8:01a Rlove
 * Change to PHASE for rotating brushes
 * 
 * 17    5/21/97 1:34p Rjohnson
 * Fix for weird problem with movement
 * 
 * 16    5/21/97 10:59a Rjohnson
 * Physics didn't allow you to change the velocity on certain "touches"
 * that occurred deep down in the physics
 * 
 * 15    5/20/97 11:21a Rlove
 * 
 * 14    5/12/97 5:07p Rjohnson
 * Made rotating bmodels also respond to velocity
 * 
 * 13    5/09/97 2:39p Rlove
 * 
 * 12    5/09/97 1:31p Rlove
 * 
 * 11    5/07/97 4:09p Rlove
 * 
 * 10    4/26/97 7:32a Rlove
 * Getting rid of some Id sounds
 * 
 * 9     3/26/97 4:05p Rjohnson
 * Added the ability to link entities so that they will move together
 * 
 * 8     3/21/97 5:57p Rlove
 * They are standing here waiting for this
 * 
 * 7     3/19/97 4:11p Rjohnson
 * Put in quake2's bmodel movement
 * 
 * 6     3/17/97 2:01p Rlove
 * Bounce missiles were sticking on the floor and that was a bad thing
 * 
 * 5     3/17/97 12:40p Rlove
 * New axe is in.
 * 
 * 4     3/07/97 2:18p Rjohnson
 * Id Updates
 * 
 * 3     2/18/97 4:40p Rjohnson
 * Id Updates
 */
