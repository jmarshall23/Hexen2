...
void archer_arrow_touch(void)
{
	...
	if (self.owner.classname == "monster_archer"||self.owner.netname=="monster_archer_ice")
		damg = random(5,7);
	else
	{
	...
	else if (self.classname == "blue_arrow")
...
void archer_launcharrow (float arrowtype,vector spot1,vector spot2)
{
	...
	else if(arrowtype==BLUE_ARROW)
	{
		sound (self, CHAN_WEAPON, "archer/arrowg2.wav", 1, ATTN_NORM);
		Create_Missile(self,spot1,spot2,"models/akarrow2.mdl","blue_arrow",0,1000,archer_arrow_touch);
	}
	else
	{
		sound (self, CHAN_WEAPON, "archer/arrowg.wav", 1, ATTN_NORM);
	...
	if (self.frame == $fire2)	// FIRE!!!!
	{
		...
		{	
			...
			if (self.netname=="monster_archer_ice")
				archer_launcharrow(BLUE_ARROW,spot1,spot2);
...
void archerdraw () [++ $tranA1..$tranA13]
{
	...
	{
		if(self.netname=="monster_archer_ice")
			sound (self, CHAN_BODY, "archer/growl4.wav", 1, ATTN_NORM);
		else if (self.classname == "monster_archer")
...
void monster_archer ()
{
	...
	if(self.netname=="monster_archer_ice")
		CreateEntityNew(self,ENT_ARCHER,"models/archer2.mdl",archer_die);
	else
...
Health : 80
Experience Pts: 25
Favorite TV shows: Northern Exposure
Favorite Color: Off-White
Likes: Shooting arrows into people and ice fishing
Dislikes: Sunshine and happiness
--------------------------------------------------------
*/
